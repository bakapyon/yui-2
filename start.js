...
try {
	if (sesConfig.Session_Multiple_Numbers == true) {
		...
		for (let i = 0; i < sesConfig.Session_ID.length; i++) {
			WA.create(tools('options').options(tools('controller').listener, sesConfig.Session_ID[i], (sesConfig.User_Data_Dir[i] || 'Another_yui_Session'))).then(kill => tools('controller').listener(kill));
		}
	} else {

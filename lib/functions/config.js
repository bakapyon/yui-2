...
const _4devs = require('@pyon/4devs');
const trendings = require('@pyon/trendings');
const tts = require('@pyon/gtts');
...
exports.kconfig = async (fun, mesg) => {
	...
	try {
		...
		/* Sistema que permite desativar a Yui em um grupo */
		...
		const stckPack = (removeAccents(config.Sticker_Pack).toString() || "🎴 Horizon - Yui");
		...
		// Impede de usar a Yui se o dono quiser apenas registrados
		...
		if (config.yui_Read_Messages) {
			try {
				await kill.sendSeen(chatId);
			} catch (error) {
				if (config.Secure_Mode) {
					config.yui_Read_Messages = false;
					fs.writeFileSync('./lib/config/Settings/config.json', JSON.stringify(config, null, "\t"));
			...
			try {
				...
				if (tools('gaming').LevelEXP(checkLvL) <= haveXptoUp) {
					...
					if (randEven.eventOnline && randEven.eventType == 'coin') {
						let what_Coin_Wins = tools('others').Bonus_Value(parseInt(tools('others').randomNumber(Number(config.yui_Coin), Number(config.yui_Coin)*randEven.events[randEven.eventIndex].multiplier)), user, chatId, 'win');
						tools('gaming').addValue(user, what_Coin_Wins, chatId, 'xp'); /* Efeito de evento aleatório */
					} else {
						let wh_Icoins = tools('others').Bonus_Value(Number(config.yui_Coin), user, chatId, 'win');
		...
		for (let i = 0; i < multiCommand.length; i++) {
			...
			switch (cmd) {
				...
				case 'reboot':
					...
					try {
						let isWorked = shell.exec(`pm2 reload yui`, {
							silent: true
						});
				...
				break;
					...
					case 'about':
					await kill.sendFileFromUrl(from, 'https://s6.gifyu.com/images/img-yui.jpg', 'yui.png', id)
				break;
					...
					try {
						if (args.length == 0 || APIS.Chat_Bot_Type == 5 && !tools('others').isUrl(APIS.SimSimi_Host) || APIS.Chat_Bot_Type == 3 && APIS.API_Brainshop == Default_APIS.API_Brainshop) {
							...
							if (ChatSpeakm == 5) {
								...
								yuiSpeak = Res_CBMFv.data.result == 100 && Res_CBMFv.data.success ? Res_CBMFv.data.success : tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0];
							} else if (ChatSpeakm == 4 && APIS.API_SimSimi) {
								const My_Cliv = await cleverbot(Sliced_Body);
								yuiSpeak = My_Cliv ? My_Cliv : tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0];
								...
								yuiSpeak = Res_CBMTr.data.cnt ? Res_CBMTr.data.cnt : tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0];
							} else if (ChatSpeakm == 2) {
								yuiSpeak = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0];
								...
								yuiSpeak = Res_CBMOn.data.success ? Res_CBMOn.data.success : tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0];
							} else yuiSpeak = tools('others').getRandLine(1, './lib/config/Utilidades/reply.txt')[0];
							if (ChatSpeakm !== 2) {
								fs.appendFileSync('./lib/config/Utilidades/reply.txt', `\n${yuiSpeak}`);
							}
							const F_Speak = await tts.create(region, yuiSpeak, false, false);
				...
				case 'roleta':
					...
					var iRoll = Math.abs(parseInt(tools('others').randomNumber(Number(config.yui_Coin), Number(args[0]) * 1.5) / Number(config.Max_Divider_Win)) + Number(args[0]));
					if (side == 1) {
						iRoll = tools('others').Bonus_Value(iRoll, user, chatId, 'lose');
						await kill.sendFileFromUrl(chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/roleta1.png', 'rol1.png', mess.loseshot(iRoll), id);
						...
						await kill.sendFileFromUrl(chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/roleta.jpg', 'rol.jpg', mess.winshot(iRoll), id);
						tools('gaming').addValue(user, Number(iRoll), chatId, 'coin');
					}
				...
				case 'flip':
					...
					var iFlip = Math.abs(parseInt(tools('others').randomNumber(Number(config.yui_Coin), Number(args[1]) * 1.5) / Number(config.Max_Divider_Win)) + Number(args[1]));
				...
				case 'cassino':
					...
					var iCass = Math.abs(parseInt(tools('others').randomNumber(Number(config.yui_Coin), Number(args[0]) * 1.5) / Number(config.Max_Divider_Win)) + Number(args[0]));
					var cassin = ['- 🍒 ', '- 🎃 ', '- 🍐 '];
				...
				case 'lgbt':
					...
					const arcoyui = await canvacord.Canvas.rainbow(theLgBtic[0]);
					await kill.sendFile(chatId, tools('others').dataURI('image/png', arcoyui), `gay.png`, mess.lgbt(lvpc, twgui[0], (100 - lvpc), twgui[1]), id);
				break;
				...
				case 'tudo':
					...
					await kill.reply(chatId, allCommands.toString(), id); // Todos as cases da Yui
				break;

				/*LEMBRE-SE, REMOVER CRÈDITO È CRIME E PROIBIDO!*/
				//case 'termos':
					//await kill.sendFileFromUrl(chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/license.png', 'licenca.png', mess.everhost() + '\n\n' + mess.tos(), id);
				...
				case '#':	
					...
					if (tttSet[chatId].thePlayerGame == 0 && tttSet[chatId].waitJogo == 0 && tttSet[chatId].isValidGame == 1 || user == tttSet[chatId].thePlayerGame && tttSet[chatId].waitJogo == 0 && tttSet[chatId].isValidGame == 1 || tttSet[chatId].thePlayerGame2 == user && tttSet[chatId].waitJogo == 0 && tttSet[chatId].isValidGame == 1) {
						...
						if (tttSet[chatId].thePlayerGameOld2 == botNumber) {
							...
							const yuiJog = (Object.keys(tttSet[chatId].tttboard)[model] || tools('others').randVal(tttSet[chatId].tictacplays));
					...
					if (gameconfig[chatId].forcw == 0 || argl[0] == '-new') {
						...
						if (awnserFind == 1) {
							...
							if (gameconfig[chatId].lives == 5) {
								await mandarFig(kill, chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/1.png', stickerConfig);
							} else if (gameconfig[chatId].lives == 4) {
								await mandarFig(kill, chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/2.png', stickerConfig);
							} else if (gameconfig[chatId].lives == 3) {
								await mandarFig(kill, chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/3.png', stickerConfig);
							} else if (gameconfig[chatId].lives == 2) {
								await mandarFig(kill, chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/4.png', stickerConfig);
							} else if (gameconfig[chatId].lives == 1) {
								await mandarFig(kill, chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/5.png', stickerConfig);
							} else if (gameconfig[chatId].lives == 0) {
								await kill.sendFileFromUrl(chatId, 'https://raw.githubusercontent.com/pyon/yui_Files/main/Assets/6.jpg', 'lose.jpg', mess.You_Lose(argl, gameconfig, chatId), id);
								return resetForca();
							}
				...
				case 'diario':
					...
					var dailyIcoin = tools('others').randomNumber(config.yui_Coin, config.yui_Coin * reward[user].rewards);
					if (randEven.eventOnline && randEven.eventType == winType) {
				...
				case 'genshin':
					...
					if (argl[0] == '-sets') {
						...
						await kill.sendFileFromUrl(chatId, `https://github.com/pyon/yui_Files/raw/main/Genshin/${charToSend}`, charToSend, mess.Best_Set(argc), id);
					} else if (argl[0] == '-char') {
				...
				case '21':
					...
					if (isGroupMsg) {
						...
						if (argl[0] == '-start' && bjConfig[chatId].isPlaying == false) {
							...
							await mandarFig(kill, chatId, `https://github.com/pyon/yui_Files/raw/main/Cards/${Played_Card}.png`, stickerConfig);
							...
							if (checkWin > 21) {
								await mandarFig(kill, chatId, `https://github.com/pyon/yui_Files/raw/main/Cards/${newCard}.png`, stickerConfig);
				...
				case 'couple':
					...
					await kill.sendFileFromUrl(chatId, `https://github.com/pyon/yui_Files/raw/main/Couple/${theCouple}.1.jpg`, 'couple.jpg', 'Você completa...', id);
					await kill.sendFileFromUrl(chatId, `https://github.com/pyon/yui_Files/raw/main/Couple/${theCouple}.2.jpg`, 'couple.jpg', '...meu coração.', id);
				break;

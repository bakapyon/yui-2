...
exports.welcomer = async (kill, events) => {
	...
	try {
		...
		// Configurações
		const yuiNumber = (await kill.getHostNumber()) + '@c.us';
		const isyui = events.who == yuiNumber;
		...
		if (events.action == 'add' && isBlacklist && !isyui) {
			...
			if (Open_Welcome.time < Date.now()) {
				...
				if (!Object.keys(customMsg[events.chat]).includes('welcome')) {
					const profile = await tools('profile').getProfilePic(kill, false, false, false, false, [events.who, events.chat], events.who, yuiNumber, groupInfo.groupMetadata.participants, 1);
					try {
			...
			if (Open_Goodbye.time < Date.now()) {
				...
				if (!Object.keys(customMsg[events.chat]).includes('goodbye')) {
					const profolo = await tools('profile').getProfilePic(kill, false, false, false, false, [events.who, events.chat], events.who, yuiNumber, groupInfo.groupMetadata.participants, 1);
					try {
